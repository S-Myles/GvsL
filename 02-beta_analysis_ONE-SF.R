...
arrows_df <- as.data.frame(scores(rda_result, display = "bp")) %>%
  ...
  filter(labels %in% c("Year2016", "Year2017", "Year2018"))
...
filt_asv_attribute_associations <- filt_asv_attribute_associations %>%
  ...
  select(Taxonomy, Year2016, Year2017, Year2018)
...
# Optional Cleanup
rm(anova_overall, anova_terms, arrows_df, asv_scores, attributes_scores, 
   filt_asv_attribute_associations, otu_clr, otu_clr_t, pca_plot_data, plot,
   rda_result, sample_scores, samples_df, taxonomy_table, variance_df, variance_plot,
   axis_labels, explained_variance, rda_variance, variance_proportions,
